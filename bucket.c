...
static zend_function_entry riak_bucket_methods[] = {
	...
	PHP_ME(RiakBucket, put, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(RiakBucket, get, NULL, ZEND_ACC_PUBLIC)
	{NULL, NULL, NULL}
};
...
PHP_METHOD(RiakBucket, get)
{
	...
	size_t contentCount;
	zval *zBucket, *zKey, *zVclock, *zConflictedExc, **zTmp, *zObjArr, *zObj;
	...
	if (riackResult == RIACK_SUCCESS) {
		...
		if (contentCount > 1) {
			...
			object_init_ex(zConflictedExc, riak_conflicted_object_exception_ce);
			CALL_METHOD2(RiakConflictedObjectException, __construct, zConflictedExc, zConflictedExc, zVclock, zObjArr);
			zend_throw_exception_object(zConflictedExc TSRMLS_CC);
			
			zval_ptr_dtor(&zObjArr);
		} else if (contentCount == 1) {
			// Create a new RiakObject
			return_value = object_from_riak_content(zKey, &getResult.object.content[0] TSRMLS_CC);
			// Update vclock on RiakObject
			zend_update_property(riak_object_ce, return_value, "vclock", sizeof("vclock")-1, zVclock TSRMLS_CC);
