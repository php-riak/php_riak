...
#define RIAK_REQ_PROP_SET_LONG(CLASS_ALIAS, OP, ZV, GETTER, TARGET_STRUCT, TARGET_NAME) \
    RIAK_CALL_METHOD(CLASS_ALIAS, GETTER, &ZV, zinput); \
    if (Z_TYPE(ZV) == IS_LONG) { \
        riak_##OP##_options_set_##TARGET_NAME(TARGET_STRUCT, Z_LVAL(ZV)); }

#define RIAK_REQ_PROP_SET_BOOL(CLASS_ALIAS, OP, ZV, GETTER, TARGET_STRUCT, TARGET_NAME) \
    RIAK_CALL_METHOD(CLASS_ALIAS, GETTER, &ZV, zinput); \
    if (Z_TYPE(ZV) == IS_BOOL && Z_BVAL(ZV)) { riak_##OP##_options_set_##TARGET_NAME(TARGET_STRUCT, Z_BVAL(ZV)); }
/*
...
PHP_METHOD(RiakBucket, index)
{
    ...
    if (to != NULL && tolen > 0) {
        rsto.len = tolen;
        rsto.value = to;
        RIACK_RETRY_OP(riackstatus, riack_2i_query_range(connection->client, rsbucket, rsindex, rsfrom, rsto, &resultlist));
    } else {
        RIACK_RETRY_OP(riackstatus, riack_2i_query_exact(connection->client, rsbucket, rsindex, rsfrom, &resultlist));
    }
    CHECK_RIACK_STATUS_THROW_AND_RETURN_ON_ERROR(connection, riackstatus);
...
PHP_METHOD(RiakBucket, indexQuery)
{
    ...
    if (Z_BVAL_P(zisrange)) {
        ...
        RIACK_RETRY_OP(riackstatus, riack_2i_query_ext(connection->client, &req, &result_keys, &continuation));
    } else {
        zval *zexact;
...
#define SET_PROPLIST_BOOL(GETTER_NAME, PROP_NAME) MAKE_STD_ZVAL(ztmp); \
    ...
    RIACK_RETRY_OP(riackResult, riack_set_bucket_props_ext(connection->client, bucketName, &properties));
...
#define GET_PROP_SET_ON_LIST(ZVAL_METHOD, SETTER_NAME, PROP_NAME) if (PROP_NAME##_use) { \
    ...
    RIACK_RETRY_OP(riackResult, riack_get_bucket_props_ext(connection->client,  bucketName, &properties));
	CHECK_RIACK_STATUS_THROW_AND_RETURN_ON_ERROR(connection, riackResult);
...
PHP_METHOD(RiakBucket, delete)
{
    ...
    if (zinput != NULL && Z_TYPE_P(zinput) == IS_OBJECT) {
        zval zget;
        /*
        riak c does not provides setters for r values
        ...
        */
        RIAK_REQ_PROP_SET_LONG(Riak_Input_DeleteInput, delete, zget, getW, opts, w);
        RIAK_REQ_PROP_SET_LONG(Riak_Input_DeleteInput, delete, zget, getDW, opts, dw);
        RIAK_REQ_PROP_SET_LONG(Riak_Input_DeleteInput, delete, zget, getPW, opts, pw);

        RIAK_CALL_METHOD(Riak_Input_DeleteInput, getVClock, &zget, zinput);
        if (Z_TYPE(zget) == IS_STRING) {
            vclock = riak_binary_deep_from_stringl(ctx->config, Z_STRLEN(zget), Z_STRVAL(zget));
        }
        zval_dtor(&zget);
    }
    if (Z_TYPE_P(zparam) == IS_OBJECT) {
        ...
        if (Z_TYPE(zkey) == IS_STRING && Z_STRVAL(zkey) > 0) {
            key = riak_binary_deep_from_stringl(ctx->config, Z_STRLEN(zkey), Z_STRVAL(zkey));
        }
        ...
        if (!vclock) {
            ...
            if (Z_TYPE(zobjvclock) == IS_STRING) {
                // Set the vclock
                vclock = riak_binary_deep_from_stringl(ctx->config, Z_STRLEN(zobjvclock), Z_STRVAL(zobjvclock));
                zval_dtor(&zobjvclock);
            }
        }
    } else if (Z_TYPE_P(zparam) == IS_STRING && Z_STRVAL_P(zparam) > 0) {
        key = riak_binary_deep_from_stringl(ctx->config, Z_STRLEN_P(zparam), Z_STRVAL_P(zparam));
    }
    if (vclock) {
        riak_delete_options_set_vclock(ctx->config, opts, vclock);
    }
    result = riak_delete(ctx->connection, bucket, key, opts);

    if (vclock) riak_binary_deep_free(ctx->config, &vclock);
    riak_binary_deep_free(ctx->config, &key);
    riak_binary_free(ctx->config, &bucket);
    riak_delete_options_free(ctx->config, &opts);
    CHECK_RIAK_STATUS_THROW_AND_RETURN_ON_ERROR(ctx, result);
}
/* }}} */
...
*/
    ...
    riak_object_properties_set(obj, zobject, ctx TSRMLS_CC);
    riak_object_set_bucket(obj, bucket);
    if (key) riak_object_set_key(obj, key);
    //
    ...
    // Why do these fail?, they should not...
...
PHP_METHOD(RiakBucket, get)
{
    ...
    MAKE_STD_ZVAL(zkey);
    ZVAL_STRINGL(zkey, skey, skeylen, 1);
    ctx = get_riak_context(getThis() TSRMLS_CC);
    bucket = binary_bucket_name_new(ctx, getThis() TSRMLS_CC);
    ...
    if (zinput != NULL && Z_TYPE_P(zinput) == IS_OBJECT) {
        zval zget;
        RIAK_REQ_PROP_SET_BOOL(Riak_Input_GetInput, get, zget, getReturnHead, opts, head);
        RIAK_REQ_PROP_SET_LONG(Riak_Input_GetInput, get, zget, getR, opts, r);
        RIAK_REQ_PROP_SET_LONG(Riak_Input_GetInput, get, zget, getPR, opts, pr);
        /*
...
*/
    ...
    if (result == ERIAK_OK) {
        zval *zout, *zResolver;
        zout = get_output_from_riack_get_response(response, zkey TSRMLS_CC);
