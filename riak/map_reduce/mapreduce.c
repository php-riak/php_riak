...
PHP_METHOD(RiakMapreduce, run)
{
    ...
    if (Z_TYPE_P(zclient) == IS_OBJECT) {
        ...
        if (zstreamer) {
            ...
            #endif
            RIACK_RETRY_OP(riackResult, connection, riack_map_reduce_stream(stream_connection->client, Z_STRLEN_P(zjson), (uint8_t*)Z_STRVAL_P(zjson), APPLICATION_JSON, &riak_mr_result_cb, &stream_params));
            CHECK_RIACK_STATUS_THROW_ON_ERROR(stream_connection, riackResult);
            release_connection(stream_connection TSRMLS_CC);
        } else {
            RIACK_RETRY_OP(riackResult, connection, riack_map_reduce(connection->client, Z_STRLEN_P(zjson), (uint8_t*)Z_STRVAL_P(zjson), APPLICATION_JSON, &mapresult));
